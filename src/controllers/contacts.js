...
exports.form = async (req, res) => {
  const mapHtml = await getSetting('contacts_map', '');
  res.locals.setMeta({
    title: 'Контакты',
  ...
  res.render('pages/contacts', { sent: req.query.sent === '1', error: null, mapHtml });
};
...
exports.submit = async (req, res) => {
  ...
  if (!name || !email || !message) {
    return res.render('pages/contacts', { sent: false, error: 'Заполните обязательные поля', mapHtml });
  }
  if (!agree) {
    return res.render('pages/contacts', { sent: false, error: 'Необходимо согласие на обработку данных', mapHtml });
  }
  if (req.body.website) {
